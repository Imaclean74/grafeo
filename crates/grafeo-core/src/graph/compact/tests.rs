...
// ===========================================================================
// F. ReadOnlyGraphStore wrapper tests
// ===========================================================================

/// Wraps a CompactStore in ReadOnlyGraphStore and exercises all forwarded methods.
fn build_read_only_store() -> crate::graph::ReadOnlyGraphStore {
    crate::graph::ReadOnlyGraphStore::new(std::sync::Arc::new(build_test_store()))
}

#[test]
fn test_readonly_get_node() {
    let store = build_read_only_store();
    let ids = store.nodes_by_label("Person");
    assert_eq!(ids.len(), 5);
    let node = store.get_node(ids[0]).unwrap();
    assert!(node.has_label("Person"));
}

#[test]
fn test_readonly_get_edge() {
    let store = build_read_only_store();
    let person_ids = store.nodes_by_label("Person");
    ...
    let (dst, eid) = edges[0];
    let edge = store.get_edge(eid).unwrap();
    assert_eq!(edge.src, person_ids[0]);
    assert_eq!(edge.dst, dst);
}

#[test]
fn test_readonly_versioned_delegates() {
    let store = build_read_only_store();
    let ids = store.nodes_by_label("Person");
    let nid = ids[0];
    // Versioned methods should delegate to non-versioned
    assert!(
        store
            .get_node_versioned(nid, EpochId(1), TransactionId(1))
            .is_some()
    );
    assert!(store.get_node_at_epoch(nid, EpochId(1)).is_some());

    let edges = store.edges_from(nid, Direction::Outgoing);
    if let Some((_, eid)) = edges.first() {
        assert!(
            store
                .get_edge_versioned(*eid, EpochId(1), TransactionId(1))
                .is_some()
        );
        assert!(store.get_edge_at_epoch(*eid, EpochId(1)).is_some());
    }
}

#[test]
fn test_readonly_property_access() {
    let store = build_read_only_store();
    let ids = store.nodes_by_label("Person");
    let age = store.get_node_property(ids[0], &PropertyKey::from("age"));
    assert!(age.is_some());

    let batch = store.get_node_property_batch(&ids, &PropertyKey::from("age"));
    assert_eq!(batch.len(), 5);
    assert!(batch.iter().all(|v| v.is_some()));
}

#[test]
fn test_readonly_batch_properties() {
    let store = build_read_only_store();
    let ids = store.nodes_by_label("Person");

    let all_props = store.get_nodes_properties_batch(&ids);
    assert_eq!(all_props.len(), 5);
    assert!(all_props[0].contains_key(&PropertyKey::from("age")));

    let selective = store.get_nodes_properties_selective_batch(&ids, &[PropertyKey::from("name")]);
    assert_eq!(selective.len(), 5);
    assert!(selective[0].contains_key(&PropertyKey::from("name")));
    assert!(!selective[0].contains_key(&PropertyKey::from("age")));
}

#[test]
fn test_readonly_edge_properties() {
    let store = build_read_only_store();
    let ids = store.nodes_by_label("Person");
    let edges = store.edges_from(ids[0], Direction::Outgoing);
    let edge_ids: Vec<EdgeId> = edges.iter().map(|(_, eid)| *eid).collect();

    let selective =
        store.get_edges_properties_selective_batch(&edge_ids, &[PropertyKey::from("weight")]);
    assert_eq!(selective.len(), edge_ids.len());
}

#[test]
fn test_readonly_neighbors_and_degree() {
    let store = build_read_only_store();
    let ids = store.nodes_by_label("Person");
    let nid = ids[0];

    let out_neighbors = store.neighbors(nid, Direction::Outgoing);
    assert_eq!(out_neighbors.len(), store.out_degree(nid));

    let in_neighbors = store.neighbors(nid, Direction::Incoming);
    assert_eq!(in_neighbors.len(), store.in_degree(nid));

    assert!(store.has_backward_adjacency());
}

#[test]
fn test_readonly_scans() {
    let store = build_read_only_store();
    assert_eq!(store.node_count(), 8);
    assert_eq!(store.edge_count(), 9);
    assert_eq!(store.node_ids().len(), 8);
    assert_eq!(store.all_node_ids().len(), 8);
    assert_eq!(store.nodes_by_label("Person").len(), 5);
    assert_eq!(store.nodes_by_label("City").len(), 3);
}

#[test]
fn test_readonly_edge_type() {
    let store = build_read_only_store();
    let ids = store.nodes_by_label("Person");
    let edges = store.edges_from(ids[0], Direction::Outgoing);
    let (_, eid) = edges[0];
    ...
    // edge_type_versioned should delegate
    let etype_v = store.edge_type_versioned(eid, EpochId(1), TransactionId(1));
    assert_eq!(etype, etype_v);
}

#[test]
fn test_readonly_search() {
    let store = build_read_only_store();
    let results = store.find_nodes_by_property("age", &Value::Int64(25));
    assert_eq!(results.len(), 1);

    let results = store.find_nodes_by_properties(&[("age", Value::Int64(25))]);
    assert_eq!(results.len(), 1);

    let range = store.find_nodes_in_range(
        "age",
        Some(&Value::Int64(30)),
        Some(&Value::Int64(40)),
        true,
    ...
    assert_eq!(range.len(), 3); // 30, 35, 40
}

#[test]
fn test_readonly_might_match() {
    let store = build_read_only_store();
    ...
    assert!(!store.has_property_index("age"));
}

#[test]
fn test_readonly_statistics() {
    let store = build_read_only_store();
    let stats = store.statistics();
    assert!(stats.total_nodes > 0);

    let card = store.estimate_label_cardinality("Person");
    assert!(card > 0.0);

    let avg = store.estimate_avg_degree("KNOWS", true);
    assert!(avg > 0.0);
}

#[test]
fn test_readonly_epoch_and_visibility() {
    let store = build_read_only_store();
    assert_eq!(store.current_epoch(), EpochId(1));
    ...
    assert!(store.is_node_visible_at_epoch(nid, EpochId(1)));
    assert!(store.is_node_visible_versioned(nid, EpochId(1), TransactionId(1)));

    let visible = store.filter_visible_node_ids(&ids, EpochId(1));
    assert_eq!(visible.len(), ids.len());

    let visible_v = store.filter_visible_node_ids_versioned(&ids, EpochId(1), TransactionId(1));
    assert_eq!(visible_v.len(), ids.len());

    let edges = store.edges_from(nid, Direction::Outgoing);
    if let Some((_, eid)) = edges.first() {
        assert!(store.is_edge_visible_at_epoch(*eid, EpochId(1)));
        assert!(store.is_edge_visible_versioned(*eid, EpochId(1), TransactionId(1)));
    }
}

#[test]
fn test_readonly_introspection() {
    let store = build_read_only_store();
    ...
    let prop_keys = store.all_property_keys();
    assert!(prop_keys.contains(&"age".to_string()));
    assert!(prop_keys.contains(&"name".to_string()));
}

#[test]
fn test_readonly_history() {
    let store = build_read_only_store();
    let ids = store.nodes_by_label("Person");
    assert!(store.get_node_history(ids[0]).is_empty());

    let edges = store.edges_from(ids[0], Direction::Outgoing);
    if let Some((_, eid)) = edges.first() {
        assert!(store.get_edge_history(*eid).is_empty());
    }
}

#[test]
#[should_panic(expected = "read-only store")]
fn test_readonly_create_node_panics() {
    let store = build_read_only_store();
...
#[test]
#[should_panic(expected = "read-only store")]
fn test_readonly_create_edge_panics() {
    let store = build_read_only_store();
    use crate::graph::traits::GraphStoreMut;
    store.create_edge(NodeId(0), NodeId(1), "TEST");
}

#[test]
#[should_panic(expected = "read-only store")]
fn test_readonly_delete_node_panics() {
    let store = build_read_only_store();
    use crate::graph::traits::GraphStoreMut;
    store.delete_node(NodeId(0));
}

#[test]
#[should_panic(expected = "read-only store")]
fn test_readonly_set_node_property_panics() {
    let store = build_read_only_store();
    use crate::graph::traits::GraphStoreMut;
    store.set_node_property(NodeId(0), "key", Value::Int64(1));
}

#[test]
#[should_panic(expected = "read-only store")]
fn test_readonly_set_edge_property_panics() {
    let store = build_read_only_store();
    use crate::graph::traits::GraphStoreMut;
    store.set_edge_property(EdgeId(0), "key", Value::Int64(1));
}

#[test]
#[should_panic(expected = "read-only store")]
fn test_readonly_remove_node_property_panics() {
    let store = build_read_only_store();
    use crate::graph::traits::GraphStoreMut;
    store.remove_node_property(NodeId(0), "key");
}

#[test]
#[should_panic(expected = "read-only store")]
fn test_readonly_add_label_panics() {
    let store = build_read_only_store();
    use crate::graph::traits::GraphStoreMut;
    store.add_label(NodeId(0), "Label");
}

// ===========================================================================
// G. Additional CompactStore coverage
// ===========================================================================
